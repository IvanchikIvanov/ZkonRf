...
class LegalScopeService:
    ...
    COUNTRY_PATTERNS = {
        ...
        "kg": ("кыргызстан", "киргизия", "kyrgyzstan", "kyrgyz", "кыргызск", "киргизск"),
    ...
    EXPLICIT_CODEX_PATTERNS = {
        "criminal": (
            "ук рф", "уголовный кодекс", "уголовного кодекса", "criminal code",
        ),
        "civil": (
            "гк рф", "гражданский кодекс", "гражданского кодекса", "civil code",
        ),
        "labor": (
            "тк рф", "трудовой кодекс", "трудового кодекса", "labor code",
        ),
        "koap": (
            "коап", "кодекс об административных", "административный кодекс",
        ),
        "family": ("ск рф", "семейный кодекс", "семейного кодекса", "family code"),
        "consumer": (
            "зозпп", "закон о защите прав потребителей", "защите прав потребителей",
        ),
        "tax": ("нк рф", "налоговый кодекс", "налогового кодекса", "tax code"),
        "housing": ("жк рф", "жилищный кодекс", "жилищного кодекса", "housing code"),
        "procedural": ("гпк", "апк", "кас", "процессуальный кодекс"),
    }
    ...
    TOPIC_PATTERNS = {
        ...
        "consumer": (
            "возврат", "вернул", "вернули", "возмест", "компенсац",
            "потребител", "магазин", "гаранти", "товар", "услуг", "услуге",
            "доставк", "заказ", "заказывал", "оформил заказ", "курьер", "позици",
            "ресторан", "кафе", "недовоз",
        ),
    ...
    LEGAL_MARKERS = (
        ...
        "развод", "краж", "наруш", "компенсац",         "ответствен", "вернул", "вернули", "доставк", "заказ", "оплат",
        "потребит", "услуг",
    )
    ...
    def _detect_intent(self, lowered: str, has_context: bool) -> str:
        ...
        # Сначала явные юридические маркеры и темы — иначе «привет, у меня доставка…» уходит в casual_chat.
        inferred_codex, topic = self._detect_inferred_codex_and_topic(lowered)
        if inferred_codex or topic:
